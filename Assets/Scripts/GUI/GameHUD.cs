    ...
    public void OnGUI()
    {
...

        {
            GUILayout.BeginVertical(Skin.GetStyle("GameHUD"));
            {
                ...
                GUILayout.Label(string.Format("Time: {0:00}:{1:00} with {2} bonus", time.Minutes + (time.Hours * 60), time.Seconds, LevelManager.Instance.CurrentTimeBonus), Skin.GetStyle("TimeText"));
            }
        }
